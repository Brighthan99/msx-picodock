...
static void print_usage(const char *prog_name) {
    ...
    printf("  -s, --sunrise  Alias for -m (kept for older scripts)\n");
    printf("  -m, --mapper   Include the PicoDock disk entry (Nextor + 192KB mapper + printer)\n");
...
void create_uf2_file(const uint8_t *data, size_t size, const char *uf2_filename) {
    ...
    fclose(uf2_file);
    /* [PicoDock] Print the file name, not the path we were handed. The wrapper
       scripts have to pass a full path (the scan is working-directory based),
       and echoing it back puts somebody else's home directory in the output of
       a tool that is shipped to strangers. */
    {
        const char *shown = strrchr(uf2_filename, '/');
        shown = shown ? shown + 1 : uf2_filename;
...
{
    // [PicoDock] the only edit to this file. The firmware embedded below is
    // PicoDock's, not stock MultiROM, and the banner should not claim otherwise
    // - it is the first thing anyone sees and the only place the difference
    // shows before a cartridge is flashed.
    printf("MSX PICOVERSE 2040 PicoDock MultiROM UF2 Creator %s\n", APP_VERSION);
    ...
    for (int i = 1; i < argc; ++i) {
        if ((strcmp(argv[i], "-s") == 0) || (strcmp(argv[i], "--sunrise") == 0)) {
            /* [PicoDock] -s used to add a disk entry without a memory mapper.
               There is no reason to want one: a machine with its own mapper is
               unaffected by carrying a second, and a machine without one cannot
               boot Nextor at all. Kept as an alias so older invocations still
               produce a working cartridge instead of an unbootable one. */
            include_mapper = true;
    ...
    if (include_sunrise) {
        ...
        strncpy(nextor_rom_name, "PicoDock Disk (Nextor)", MAX_FILE_NAME_LENGTH);
        ...
        printf("File %02d: Name = %-50s, Size = %07u bytes, Flash Offset = 0x%08X, Mapper = %s\n",
               file_index, "PicoDock Disk (Nextor)", nextor_size, nextor_offset, mapper_description(10));
    ...
    if (include_mapper) {
        ...
        strncpy(mapper_rom_name, "PicoDock Disk (Nextor)", MAX_FILE_NAME_LENGTH);
        ...
        config_offset += sizeof(nextor_offset);
        printf("File %02d: Name = %-50s, Size = %07u bytes, Flash Offset = 0x%08X, Mapper = %s\n",
               file_index, "PicoDock Disk (Nextor)", nextor_size, nextor_offset, mapper_description(11));
        total_rom_size += nextor_size;
        if (total_rom_size > MAX_TOTAL_ROM_SIZE) {
            printf("Total ROM data exceeds maximum supported size of %u bytes.\n", (unsigned)MAX_TOTAL_ROM_SIZE);
            free(config_buffer);
