...
// Printer I/O servicing for the Sunrise/DOS loop.
//
// Drains the PIO1 I/O FIFOs and hands writes to the printer capture. Writes to
// 0x90/0x91 are the ones it acts on (pd_stdprint_io_write ignores the rest);
// every other I/O write - VDP, PSG, RTC - passes through harmlessly.
//
// Only the write captor is drained. The I/O read responder is left disabled in
// this mode (see loadrom_sunrise): with no in-PIO port filter it would forward
...
static inline void __not_in_flash_func(status_knock_write)(uint16_t port, uint8_t data)
{
    ...
    // If these do not show up on the host, the write never got here and the
    // fault is upstream (captor/FIFO/drain), not in the responder.
...
static inline void __not_in_flash_func(drain_io_printer)(void)
{
    uint16_t io_addr;
    uint8_t  io_data;
...
static inline void __not_in_flash_func(handle_menu_write)(uint16_t addr, uint8_t data, void *ctx)
{
    ...
    {
        // [PDSER] byte to the host / RX ACK (0xBFF2, 0xBFF3)
        pd_mb_write(addr, data);
    }
}
...
int __no_inline_not_in_flash_func(loadrom_msx_menu)(uint32_t offset)
{
    ...
    uint32_t menu_reads_served = 0;   // [PDSER] for the diagnostic heartbeat
    ...
    while (true)  // Loop until a ROM is selected
    {
        ...
        else
        {
            ...
            // [PDSER] The mailbox takes precedence over ROM serving (0xBFF0-0xBFF3).
            // Menu mode only — while a game runs those addresses are the game's.
            if (pd_mb_is_addr(addr))
            {
                pio_sm_put_blocking(msx_bus.pio, msx_bus.sm_read,
                                    pio_build_token(true, pd_mb_read(addr)));
                continue;
            }
            ...
            // [PDSER] Link diagnostic — retires itself once the MSX uses the mailbox
            pd_mb_diag_tick(++menu_reads_served);
        }
    }
}
...
static inline void __not_in_flash_func(handle_sunrise_write)(uint16_t addr, uint8_t data, void *ctx)
{
    ...
    // [PDSER] host mailbox first (0x7F00-0x7F0B) - see pd_mailbox.h
    if (pd_dos_is_addr(addr))
    {
        pd_dos_write(addr, data);
        return;
    }
...
void __no_inline_not_in_flash_func(loadrom_sunrise)(uint32_t offset, bool cache_enable)
{
    ...
    // [PDSER] Core1 already runs the CDC device task (launched in main), and the
    // USB host stack is not built, so the Sunrise USB storage backend is gone.
    // Left as a no-op call so the IDE register emulation path stays intact.
    sunrise_usb_set_ide_ctx(&ide);

    // Initialise PIO bus engine
    msx_pio_bus_init();

    // [PRINTER] Bring up the I/O port engine (PIO1) so 0xF5/0xF6 writes are
    // captured. Only the write captor matters here - it samples pins and never
    // drives the bus, so it does not fight PIO0 for the D0-D7 / WAIT mux. This
    // is what lets a DOS session booted from the virtual disk also print.
    //
    // The read responder that msx_pio_io_bus_init() also starts is disabled at
    // once: it has no in-PIO port filter, so leaving it on floods core0 with
    // every I/O read in the machine. Only the write side is wanted here.
    msx_pio_io_bus_init();
    pio_sm_set_enabled(msx_io_bus.pio_read, msx_io_bus.sm_io_read, false);
    ...
    while (true)
    {
        ...
        if (in_window)
        {
            // [PDSER] The host mailbox sits at 0x7F00-0x7F0B, which the Nextor ROM
            // leaves unused in every segment. It has to live inside this window
            // because Nextor mode only serves 0x4000-0x7FFF - 0xBFF0 (menu mode)
            // is unreachable from MSX-DOS.
            uint8_t ide_data;
            if (pd_dos_is_addr(addr))
            {
...
void __no_inline_not_in_flash_func(loadrom_sunrise_mapper)(uint32_t offset, bool cache_enable)
{
    ...
    // read SM, which releases WAIT as hi-Z until it needs to stretch a read,
    ...
    while (true)
    {
            ...
            while (pio_try_get_write(&waddr, &wdata))
            {
                ...
                else
                {
                    ...
                    if (active_subslot == 0)
                    {
                        ...
                        {
                            // [PDSER] The mailbox is two-way: the MSX writes a
                            // request here and reads the answer back from the
                            // same window. Serving only the read half would make
                            // the signature appear and every exchange after it
                            // hang, which is a worse failure than not being
                            // found at all.
        ...
        // --- Drain I/O writes (mapper page registers FC-FF, and the printer) ---
        // I/O ports are global (not slot-dependent), so the mapper must
        // always track writes to FC-FF regardless of subslot selection.
        //
        // [PRINTER] The same FIFO carries the printer's writes. The captor has
        // no in-PIO port filter - it forwards every I/O write in the machine -
        // so both features read one stream and each picks out its own ports.
        // This is why the mapper and the printer can run together at all; the
        // note that said they could not assumed a second capture engine was
        // needed, and there is only ever one.
        {
            uint16_t io_addr;
            uint8_t io_data;
        ...
        // always respond to FC-FF reads regardless of subslot selection.
        {
            ...
            while (pio_try_get_io_read(&io_addr))
            {
                ...
                // [PRINTER] 0x90 is deliberately NOT answered from here. This
                // loop can compute a byte but cannot put it on the bus: D0-D7
            ...
            else
            {
                ...
                if (active_subslot == 0)
                {
                    if (addr >= 0x4000u && addr <= 0x7FFFu)
                    {
                        ...
                        // [PDSER] The host mailbox at 0x7F00-0x7F0B, exactly as
                        // loadrom_sunrise serves it. Without this branch the
                        // PDSERIAL signature is not there, and every MSX-side
                        // tool that finds the cartridge by scanning slots for it
                        // - PDINFO, PDASK - reports "No PicoDock found". It has
                        // to be inside this window: Nextor mode only serves
                        // 0x4000-0x7FFF, so the menu-mode address is unreachable
                        // from MSX-DOS.
                        uint8_t ide_data;
                        if (pd_dos_is_addr(addr))
                        {
