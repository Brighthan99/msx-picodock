...
#ifndef PICODOCK_VERSION
#define PICODOCK_VERSION "v0.10"
#endif
...
// [PDSER] The original read its arguments off the stack (`ld hl,#2 / add hl,sp`),
// which assumes the pre-sdcc-4.2 sdcccall(0) convention. Under sdcc 4.5's default
// (sdcccall(1)) arguments arrive in registers, so it read garbage and painted
// broken characters on screen (the cause of the failed 2026-07-23 hardware test B).
//
// Adding `__sdcccall(0)` would make callers push again, but whether the stack
// layout matches can only be confirmed on real hardware. So instead the arguments
// are passed through static variables and the code **does not depend on the
// calling convention at all** — it behaves the same however it is compiled.
static unsigned char blit_arg_row;
static const char  *blit_arg_src;

static void blit_row_vram_do(void) __naked
{
    __asm
    ld      a, (_blit_arg_row)
    ld      b, a           ; B = row
...
// Convention-independent wrapper: hand the arguments over in static variables.
static void blit_row_vram(unsigned char row, const char *src)
{
    blit_arg_row = row;
    blit_arg_src = src;
    blit_row_vram_do();
...
// [PDSER] This one must follow the *new* convention (sdcccall(1)), because its
// caller is sdcc 4.5's standard printf. Under that convention an int argument
// arrives in HL. (The original read the stack with `add hl,sp` and therefore
// printed garbage under sdcccall(1).)
int putchar (int character) __naked
{
    (void)character;
    __asm
    ld      a, l                 ;sdcccall(1): 16-bit arg in HL, char is the low byte
    ...
    ld      hl, #0               ;return value (printf ignores it)
    ret
    __endasm;
}
...
static void copy_menu_text(char *dst, const char *src, unsigned char width, unsigned char inverted)
{
    for (unsigned char i = 0; i < width; i++) {
        unsigned char ch = src[i] ? (unsigned char)src[i] : ' ';
...
static void render_menu_row(unsigned int recordIndex, unsigned char row, unsigned char selected)
{
    ...
    copy_menu_text(&buffer[MENU_MAPPER_COL], mapper, 6, 0);
...
// [PDSER] __sdcccall(0) — returns its value in L (`ld l,a`), the old convention.
static unsigned char bios_chsns(void) __naked __sdcccall(0)
{
    __asm
    ld      iy,(#BIOS_EXPTBL-1)
    push    ix
...
// [PDSER] __sdcccall(0) — returns its value in L (`ld l,a`), the old convention.
static unsigned char bios_chget(void) __naked __sdcccall(0)
{
    __asm
    ld      iy,(#BIOS_EXPTBL-1)
    push    ix
...
void displayMenu() {
    ...
    Locate(0, 0);
    printf("MSX PICOVERSE 2040   [PicoDock %s]", PICODOCK_VERSION);
    Locate(0, 1);
...
// ---------------------------------------------------------------------------
// [PDSER] D1a — mailbox round-trip diagnostic screen
// ---------------------------------------------------------------------------
// The top 4 bytes of the cartridge window are the window onto the byte pipe to
// the host (see pd_mailbox.h):
//
//   0xBFF0 R  STATUS   bit0 RX_READY / bit1 TX_FULL / bit2 USB_CONNECTED
//   0xBFF1 R  RX_DATA  byte from the host (reading does not move the queue)
//   0xBFF2 W  TX_DATA  one byte MSX -> host
//   0xBFF3 W  RX_ACK   consume the current RX byte, advance to the next
//
// Three things are checked here; no protocol is layered on yet:
//   (1) address decoding : STATUS is not 0xFF, and bit2 sets when USB is up
//   (2) MSX -> host       : a keypress shows up in the host terminal
//   (3) host -> MSX       : a character sent from the host appears on screen
//
// ESC returns to the menu.
static void put_hex_byte(unsigned char v)
{
    const char *digits = "0123456789ABCDEF";
    PrintChar(digits[(v >> 4) & 0x0F]);
    PrintChar(digits[v & 0x0F]);
}

void mailboxTest()
{
    unsigned char st, prev_st = 0xFE;
    unsigned char rx_col = 0;

    clear_screen_0();
    Locate(0, 0);
    printf("PDSER MAILBOX TEST (D1a)");
    Locate(0, 1);
    printf("ESC=exit  keys->host  host->screen");
    Locate(0, 3);
    printf("STATUS 0xBFF0:");
    Locate(0, 5);
    printf("from host:");

    while (1)
    {
        // (1) address decoding — keep STATUS on screen
        st = Peek(PD_MB_STATUS);
        if (st != prev_st)
        {
            prev_st = st;
            Locate(15, 3);
            ...
            Locate(19, 3);
            printf("%s%s%s",
                   (st & PD_ST_USB_CONN) ? "USB " : "--- ",
                   (st & PD_ST_RX_READY) ? "RX "  : "-- ",
                   (st & PD_ST_TX_FULL)  ? "FULL" : "----");
        }

        // (3) host -> MSX — when RX_READY, read it, print it, then ACK
        if (st & PD_ST_RX_READY)
        {
            unsigned char c = Peek(PD_MB_RX_DATA);
            Poke(PD_MB_RX_ACK, 0);          // signal consumption (the read had no side effect)

            if (c == 13 || c == 10 || rx_col >= 38)
            {
                rx_col = 0;
                Locate(0, 7);
                printf("%s", "                                       ");
            ...
            {
                Locate(rx_col, 7);
                PrintChar((c >= 32 && c < 127) ? c : '.');
                rx_col++;
            }
            prev_st = 0xFE;                    // force the STATUS line to redraw
        }

        // (2) MSX -> host — send one byte per keypress
        if (bios_chsns())
        {
            unsigned char k = bios_chget();
            if (k == 27) break;                // ESC
            if (!(Peek(PD_MB_STATUS) & PD_ST_TX_FULL))
                Poke(PD_MB_TX_DATA, k);
            Locate(0, 9);
            printf("sent: ");
            put_hex_byte(k);
        }
    ...
    while (1) 
    {
        ...
        switch (key) 
        {
            ...
            case 27: // ESC
                ...
                break;
            case 77:  // M - [PDSER] mailbox diagnostic (D1a)
            case 109: // m
                mailboxTest();
                return;   // mailboxTest redraws the menu when it returns
